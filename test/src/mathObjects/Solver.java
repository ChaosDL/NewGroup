...
//		return new Equation(leftSide, rightSide);
	...
	private static ArrayList<Term> createAndAddTerms(String termString){
		...
		int i = 0;//index first digit (in case the String starts with "-")
		boolean positiveTerm = true;//assume first term is positive
		if(termString.startsWith("-")){
			positiveTerm = false;
			i++;
		}
		while(termString.length() > 0){//delete one term at a time as it gets term
	...
	public static String getConfirm(Equation eq) {
		...
		return "Your message was interpreted as " + eq.toString() + " You good?";
	...
	public static void solve(Equation eq) {
		...
		eq.setCancelRight(sideR);
		if(sideR){
			eq.toZeroOnOneSide(eq.getRightSide());
			gui.addStep(eq, "to zero on right");
		}
		else{
			eq.toZeroOnOneSide(eq.getLeftSide());
			gui.addStep(eq, "to zero on left");
		}
		eq.simplify(eq.getLeftSide());
		gui.addStep(eq, "simp left");
		eq.simplify(eq.getRightSide());
		gui.addStep(eq, "simp right");
		if(eq.isLinear()){
			if(sideR)eq.solveLinear(eq.getLeftSide());
			else eq.solveLinear(eq.getRightSide());
			gui.addStep(eq, "yes");
		}
		else if(eq.isQuadratic()){
			ArrayList<Term> theFactors = null;
			if(sideR)theFactors = eq.solveQuadratic(eq.getLeftSide());
			else theFactors = eq.solveQuadratic(eq.getRightSide());
			gui.addStep(eq, "separate term then factor");
			if(theFactors.size() > 3){
				gui.addStep("(" + theFactors.get(0).toString() + " + " + theFactors.get(1).toString() + ")(" + theFactors.get(2).toString() + " + " + theFactors.get(3).toString() + ")");
				double ans1 = (theFactors.get(1).getAddInverse().getCoefficient())/(theFactors.get(0).getCoefficient());
				double ans2 = (theFactors.get(3).getAddInverse().getCoefficient())/(theFactors.get(2).getCoefficient());
				String var = theFactors.get(0).getVariable();
				gui.addStep(var + " = " + ans1);
				gui.addStep(var + " = " + ans2);
			}
			else{
				QuadraticForm qf = new QuadraticForm(""+ -theFactors.get(1).getCoefficient(), ""+theFactors.get(2).getCoefficient(), ""+ 2*theFactors.get(0).getCoefficient());
				gui.addStep(qf, "Not rational roots so ye");
			}
